game/map/region{
	...
	proc{
		enemies(wave, boss){
			...
			if(istype(strong)){ strong = pick(strong)}
			E.Add(strong)
			var/list/medium = medium_enemies[medium_index]
			if(istype(medium)){ medium = pick(medium)}
			E.Add(medium)
			var/list/weak = weak_enemies[weak_index]
			if(istype(weak)){ weak = pick(weak)}
			E.Add(weak)
			return E
			}
		}
	...
	pyramid{
		icon = 'pyramid.dmi'
		weak_enemies = list(
			list(/game/enemy/snake/_1,/game/enemy/snake/_2,/game/enemy/snake/_3),
			/game/enemy/snake/_1,
			/game/enemy/snake/_2,
			/game/enemy/snake/_3,
			/game/enemy/mummy/_1,
	...
	//	*Skeletons, Skull Knights, *Spirits
		}
	cave{
		icon = 'cave.dmi'
		weak_enemies = list(
